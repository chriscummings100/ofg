...
TEST_CASE("pipeline cache key compares target formats material layout and shader revision") {
    const ofg::PipelineKey key{WGPUTextureFormat_RGBA8Unorm,
        WGPUTextureFormat_Depth24Plus,
        fake_bind_group_layout(1),
        fake_bind_group_layout(2),
        7};

    CHECK(key == ofg::PipelineKey{WGPUTextureFormat_RGBA8Unorm,
                     WGPUTextureFormat_Depth24Plus,
                     fake_bind_group_layout(1),
                     fake_bind_group_layout(2),
                     7});
    CHECK((key == ofg::PipelineKey{WGPUTextureFormat_BGRA8Unorm,
                      WGPUTextureFormat_Depth24Plus,
                      fake_bind_group_layout(1),
                      fake_bind_group_layout(2),
                      7}) == false);
...
TEST_CASE("pipeline cache rejects incomplete creation inputs") {
    ...
    const ofg::PipelineKey bad_format{WGPUTextureFormat_Undefined,
        WGPUTextureFormat_Depth24Plus,
        fake_bind_group_layout(1),
        fake_bind_group_layout(2),
        1};
    ...
    const ofg::PipelineKey missing_shadow_layout{
        WGPUTextureFormat_RGBA8Unorm, WGPUTextureFormat_Depth24Plus, fake_bind_group_layout(1), nullptr, 1};
    CHECK_THROWS_WITH_AS(([&]() {
        (void)cache.get_or_create(fake_device(5),
            missing_shadow_layout,
            fake_bind_group_layout(2),
            fake_bind_group_layout(3),
            fake_shader_module(4));
    }()),
        doctest::Contains("bind group layouts"),
        ofg::EngineError);
}
