...
TEST_CASE("GameRuntime records resize and tick state") {
    ...
    CHECK(status.m_canvas_width == 800);
    CHECK(status.m_canvas_height == 450);
    CHECK(status.m_device_pixel_ratio == doctest::Approx(1.5));
    CHECK(status.m_frame_count == 2);
    CHECK(status.m_last_error.has_value() == false);
}
...
TEST_CASE("GameRuntime tracks GPU readiness and target configuration") {
    ...
    CHECK(runtime.status().m_initialized == false);

    REQUIRE(runtime.mark_surface_configured(error));
    CHECK(runtime.status().m_initialized == true);
    CHECK(runtime.status().m_surface_configure_count == 1);
    CHECK(runtime.status().m_adapter_name == "adapter");
    CHECK(runtime.status().m_backend == "Backend");
    CHECK(runtime.status().m_surface_format == "Rgba8Unorm");
}
...
TEST_CASE("GameRuntime keeps zero-size targets recoverable") {
    ...
    CHECK(runtime.status().m_initialized == false);
    CHECK(runtime.status().m_surface_configure_count == 0);
    CHECK(runtime.status().m_last_error.has_value() == false);
...
TEST_CASE("GameRuntime counts target configuration transitions once") {
    ...
    CHECK(runtime.status().m_initialized == false);
    REQUIRE(runtime.mark_surface_configured(error));
    CHECK(runtime.status().m_surface_configure_count == 2);
}
...
TEST_CASE("GameRuntime records durable renderer resource counters") {
    ...
    CHECK(runtime.status().m_pipeline_create_count == 1);
    CHECK(runtime.status().m_buffer_create_count == 1);
    CHECK(runtime.status().m_last_error.has_value() == false);
}
...
TEST_CASE("GameRuntime recoverable errors preserve configured target state") {
    ...
    CHECK(runtime.status().m_initialized == false);
    REQUIRE(runtime.status().m_last_error.has_value());
    CHECK(*runtime.status().m_last_error == "transient render validation");
...
TEST_CASE("GameRuntime rejects invalid tick and resize inputs") {
    ...
    CHECK(runtime.status().m_canvas_width == 800);
    REQUIRE(runtime.status().m_last_error.has_value());
    CHECK(*runtime.status().m_last_error == error);

    CHECK(runtime.tick(std::numeric_limits<double>::infinity(), error) == false);
    CHECK(error.find("Frame time") != std::string::npos);
    CHECK(runtime.status().m_frame_count == 0);
}
...
TEST_CASE("GameRuntime dispose blocks future mutation") {
    ...
    CHECK(runtime.status().m_frame_count == 1);
    ...
    REQUIRE(runtime.status().m_last_error.has_value());
    CHECK(*runtime.status().m_last_error == "Game runtime has been disposed.");
...
TEST_CASE("RenderTarget validation rejects invalid frame targets") {
    ...
    CHECK(ofg::validate_render_target(ofg::RenderTarget{}, expected_format, 800, 450, error) == false);
    CHECK(error.find("texture view") != std::string::npos);

    CHECK(ofg::validate_render_target(ofg::RenderTarget{fake_texture_view(), WGPUTextureFormat_BGRA8Unorm, 800, 450},
              expected_format,
              800,
              450,
              error) == false);
    ...
    CHECK(ofg::validate_render_target(
              ofg::RenderTarget{fake_texture_view(), expected_format, 801, 450}, expected_format, 800, 450, error) ==
          false);
    CHECK(error.find("does not match latest resize") != std::string::npos);

    CHECK(ofg::validate_render_target(
              ofg::RenderTarget{fake_texture_view(), expected_format, 0, 450}, expected_format, 800, 450, error) ==
          false);
...
TEST_CASE("RenderTarget validation accepts matching nonzero target") {
    ...
    CHECK(ofg::validate_render_target(ofg::RenderTarget{fake_texture_view(), WGPUTextureFormat_RGBA8Unorm, 800, 450},
...
TEST_CASE("Game create validates color format and GPU handles") {
    ...
    CHECK(ofg::Game::create(ofg::GpuContext{}, WGPUTextureFormat_RGBA8Unorm, error) == nullptr);
    CHECK(error.find("device and queue") != std::string::npos);

    CHECK(ofg::Game::create(ofg::GpuContext{}, WGPUTextureFormat_Undefined, error) == nullptr);
