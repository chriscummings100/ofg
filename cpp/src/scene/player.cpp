...
// Returns the entity's local forward direction in right-handed scene space.
...
void Player::update(const SceneUpdateContext& context) {
    ...
    const float distance = m_walk_speed * speed_multiplier(context.m_controls) * context.m_delta_seconds;
