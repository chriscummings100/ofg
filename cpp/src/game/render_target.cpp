...
bool validate_render_target(
  ...
  if (target.format != expected_format) {
    error = "Game render target format " + gpu::texture_format_name(target.format) +
            " does not match renderer format " +
            gpu::texture_format_name(expected_format) + ".";
    return false;
  }
  ...
  if (target.width != expected_width || target.height != expected_height) {
    ...
    out << "Game render target size " << target.width << "x" << target.height
        << " does not match latest resize " << expected_width << "x"
        << expected_height << ".";
    error = out.str();
    return false;
  }
