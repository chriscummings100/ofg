...
std::string surface_texture_status_name(WGPUSurfaceGetCurrentTextureStatus status) {
    switch (status) {
...
std::string failure_message(const char* operation, const std::string& status, WGPUStringView message) {
...
WGPUTextureFormat choose_surface_format(const WGPUSurfaceCapabilities& capabilities) {
    constexpr std::array preferred_formats{WGPUTextureFormat_BGRA8Unorm,
