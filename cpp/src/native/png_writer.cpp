...
void append_chunk(std::vector<std::uint8_t>& png, const std::array<char, 4>& type, std::span<const std::uint8_t> data) {
    ...
    append_u32_be(png, crc32(std::span<const std::uint8_t>{png.data() + crc_start, 4 + data.size()}));
}
...
std::vector<std::uint8_t> build_scanlines(
    ...
    const std::size_t bytes_per_row = static_cast<std::size_t>(width) * _bytes_per_pixel;
    const std::size_t expected = bytes_per_row * height;
    if (rgba.size() != expected) {
        throw std::runtime_error(
            "Expected " + std::to_string(expected) + " RGBA bytes, got " + std::to_string(rgba.size()) + ".");
    }
...
std::vector<std::uint8_t> build_zlib_stored_stream(std::span<const std::uint8_t> bytes) {
