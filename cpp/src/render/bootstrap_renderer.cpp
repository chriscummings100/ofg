...
constexpr char kBootstrapShaderSource[] = R"wgsl(
struct VertexOutput {
...
WGPUShaderModule create_shader_module(WGPUDevice device, std::string& error) {
  ...
  shader_source.code = gpu::cstring_view(kBootstrapShaderSource);
...
WGPUBuffer create_vertex_buffer(
  WGPUDevice device,
  WGPUQueue queue,
  std::string& error
) {
...
BootstrapRenderer::BootstrapRenderer(
  ...
  : pipeline_(pipeline),
    ...
    counters_(RendererCounters{1, 1}) {
}
...
std::unique_ptr<BootstrapRenderer> BootstrapRenderer::create(
  ...
  WGPURenderPipeline pipeline =
    create_render_pipeline(device, format, shader, layout, error);
  ...
  return std::unique_ptr<BootstrapRenderer>(
    new BootstrapRenderer(pipeline, vertex_buffer)
  );
}

// Encodes a clear+draw pass into the caller-provided command encoder.
bool BootstrapRenderer::render_to_view(
  ...
  if (pipeline_ == nullptr || vertex_buffer_ == nullptr) {
  ...
  wgpuRenderPassEncoderDraw(
    pass,
    static_cast<std::uint32_t>(bootstrap_vertices().size()),
    1,
    0,
    0
  );
