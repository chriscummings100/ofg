...
- `cpp/src/debug/debug_ui.cpp`: Dear ImGui/WebGPU bridge code. The GPU-free debug registry remains in the native line gate, but `DebugUi` behavior depends on a live ImGui context, WebGPU backend, command encoder, and final render target, so it is covered by `npm run smoke:browser`, `npm run smoke:browser:cpp`, and `npm run smoke:render`.
