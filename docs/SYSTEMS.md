...
- `BrowserGame` accepts selected player model and animation-library bytes from TypeScript. If bytes arrive before async WebGPU setup has created `Game`, or before `Game::prepare()` has created the player scene, `BrowserGame` queues them until `Game` is ready to delegate import to the `Player` component.
