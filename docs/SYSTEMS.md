...
The C++ runtime is the C++/WASM facade exposed to TypeScript. `BrowserGame` owns the browser-specific frame driver: WebGPU instance/adapter/device/surface setup, generic blob-load polling/completion methods, surface resize policy, setup-phase control-input buffering, surface texture acquisition, command encoder creation, command-buffer finish, queue submit, and Embind lifecycle ownership. `Game` exposes a static public lifecycle backed by one private singleton instance: single-shot `create`, repeated `prepare`, `resize`, `update`, `render`, repeated `release`, and single-shot `destroy`. `Resources` and `Renderer` expose their own static lifecycles. `Resources` owns active high-level resource storage, blob request/cache state, the canonical URI registry for loadable resources, `ModelResource` objects, all durable imported mesh/material/texture/shader resources, observable `Resource` load state, and a generic loading-resource scheduler for the borrowed WebGPU device; `Renderer` owns pass creation, pass lifetime, resize propagation, command recording, transient render-object/draw-list construction, shadow target/pass ownership, and renderer counters. `Game` owns frame counting, debug-status serialization, the current `Scene` pointer, the latest raw `ControlInput` snapshot, demo-scene binding state, model-loading status, target validation, and the active scene passed to `Renderer::render`. The `Player` component owns hardcoded default player model-resource handles, instantiated model root, fallback renderer visibility, and locomotion animation binding.
...
- `cpp/include/ofg/render/renderer.hpp` exposes the static `Renderer` lifecycle facade, scene render entry point, and read-only bloom, shadow-pass, and temp-buffer diagnostics.
- `cpp/include/ofg/scene/scene.hpp` exposes the first ECS scene graph: a root entity tree, local transforms, scene-owned component containers, indexed `MeshRenderer` iteration, scene-owned `Camera` selection, scene-owned `Environment`, and scene-owned `Terrain`.
- `cpp/include/ofg/terrain/terrain.hpp` and `cpp/include/ofg/terrain/terrain_chunk.hpp` expose deterministic terrain sampling, LOD0 terrain chunk constants, the `(LOD, X, Y, Z)` chunk id, origin-centred terrain ticking, and generated chunk heightfield samples.
- `cpp/include/ofg/terrain/terrain_scene.hpp` exposes the first terrain render-resource bridge that realizes height-debug textures/materials and retargets fixed debug draw slots from scene-owned terrain chunks.
...
- The `debug_status_json()` fields are a public inspection contract for TypeScript tests, Playwright smoke, and later diagnostics. The current render-inspection fields include default demo-scene identity/count diagnostics, render-culling extracted/visible/culled counts, durable renderer counters, `shadow` pass/cascade/caster diagnostics, `bloomActiveLevelCount`, `bloomEncodedPassCount`, `bloomEstimatedReadBytes`, `bloomEstimatedWriteBytes`, `bloomSkipped`, and `tempBuffer*` memory/reuse counters.
...
TerrainGeneration owns the first procedural terrain data model. It is CPU-side and renderer-ready with the first single-LOD streaming shape: `Scene` owns one `Terrain` object, `Terrain` owns deterministic `TerrainConfig`, and `Terrain` owns a `std::map` of currently streamed `TerrainChunk` objects keyed by `TerrainChunkId{LOD, X, Y, Z}`. LOD0 is the only supported level for now. Each LOD0 chunk represents 32 by 32 by 32 one-meter cells and generates a 33 by 33 X/Z heightfield surface so adjacent chunks share boundary samples. Each tick reconciles a 5 by 5 surface region around a supplied generation origin, using X/Z radius 2, `lod = 0`, and `chunk_y = 0`.

Public interfaces:

- `cpp/include/ofg/terrain/terrain.hpp` exposes `TerrainConfig`, `TerrainTickContext`, `Terrain::sample`, `Terrain::tick`, chunk lookup, and current chunk iteration.
- `cpp/include/ofg/terrain/terrain_chunk.hpp` exposes `TerrainChunkId`, fixed LOD0 constants, `TerrainSample`, `TerrainChunk`, chunk-id validation, heightfield sample access, and render-resource pointer slots for debug textures, debug planes, and later generated meshes.
- `cpp/include/ofg/terrain/terrain_scene.hpp` exposes `TerrainSceneResources`, `terrain_height_debug_shader_layout`, `build_terrain_debug_resources`, `setup_terrain_debug_scene`, and `sync_terrain_debug_scene`.
- `cpp/include/ofg/scene/scene.hpp` exposes `Scene::terrain()` and `Scene::terrain() const`.
- `cpp/tests/terrain_test.cpp` and `cpp/tests/terrain_chunk_test.cpp` pin deterministic sampling, floor-division chunk lookup, idempotent ticking, scene ownership, invalid input handling, fixed sample counts, and adjacent edge agreement.

Communication contracts:

- Terrain generation is C++ owned. TypeScript and the WASM facade must not receive bulk terrain data or terrain editor state in this milestone.
- `Terrain::sample(world_x, world_z)` is deterministic for the same config and world coordinate. The generation origin only decides which chunks should exist; it must not change sample values.
- Chunk coordinates use floor division for negative coordinates. World `x` in `[0, 32)` maps to chunk `0`, and world `x` in `[-32, 0)` maps to chunk `-1`.
- `Terrain::tick` is idempotent for an unchanged origin region. Repeated ticks should not duplicate chunks. When the origin moves, Terrain unloads chunks outside the target region and creates any newly needed chunks.
- `Scene::update` ticks terrain after player and animation-player updates, using the primary player position when present and origin otherwise, so terrain generation is ready before render extraction and later streaming work.
- The Milestone 1 render bridge attaches one `R16Float` 33 by 33 height-debug texture to each visible chunk, reuses one shared 32 by 32 meter debug-plane mesh, and retargets a fixed set of scene draw slots to the current chunks at each chunk's `y == 0` X/Z footprint.
- Compatible material bind-group layouts are shared across materials with the same shader/material binding structure so per-chunk terrain debug textures do not create one opaque pipeline per chunk material.
- Future terrain mesh work should attach generated meshes to the existing `TerrainChunk` identities rather than introducing parallel mesh-only terrain ownership.

## CppRenderer

The C++ renderer owns the current pass-based draw-list renderer used behind static `Game` in browser and native frame drivers. `Renderer` is a static lifecycle facade that creates and owns explicit render targets and passes: an `RGBA16Float` scene-color target, a shared depth target, a cascaded shadow-map depth target, the shadow caster pass, the opaque pass, the procedural sky pass, the bloom post effect, and the final tone-map pass. `TempBuffer` is a renderer-internal static lifecycle facade for reusable temporary render-target textures and views; passes ask for exact descriptor matches with `TempBuffer::get(...)`, may return a handle early with `TempBuffer::release(buffer)` after final encoded GPU use, and otherwise rely on `TempBuffer::end_frame()` to make active scratch targets reusable. `Resources` owns active high-level resource storage, `ModelResource` objects, resource load state, blob cache state, the generic loading-resource scheduler, and the borrowed `GpuContext`; `Game` uses that facade to build demo resources, then maintains a current ECS `Scene` containing scene-owned `Camera`, `Player`, `AnimationPlayer`, `MeshRenderer`, and `Light` components plus scene-owned `Environment` and `Terrain`. OFG scene/game space is left-handed with `+X` right, `+Y` up, and `+Z` forward; cameras use local `+Z` as forward, terrain height maps to `+Y`, directional lights use entity world `+Z` as the light travel direction, and `CameraProperties` / `LightProperties` resolve those conventions into renderer-facing values. `Environment` owns ambient lighting, weather controls, day/night state, deterministic daylight/sunset/night/storm presets, moon/star state seeds, and a safe pointer to the current sun directional light. `Scene::update` updates `Environment`, players, animation players, terrain, CPU-skinned mesh renderers, then cameras so same-frame camera follow observes player movement and rendering sees current skinned vertices. `Player` owns the default player model import/instantiation state and writes locomotion clip weights from current speed before animation sampling. `Camera` owns debug, first-person, and third-person mode behavior for the single active camera entity. `CameraProperties` is the renderer-facing camera snapshot that carries resolved camera matrices, source camera, aspect, and clip distances. `Renderer::render` resolves `Scene::main_camera()` into `CameraProperties`, extracts authored-visible scene mesh renderers into bounded transient render objects, camera-frustum culls those render objects into a private transient `DrawList`, builds a transient `LightProperties` list from `scene.environment().main_directional_light()`, builds three stable sun-shadow cascades, culls shadow casters from the full extracted render-object list per cascade, records depth-only current-sun shadow caster draws into a three-layer `Depth32Float` shadow map when shadows are active, packs a `ShadowFrameState` for the opaque pass, records opaque draws that sample the shadow-map array for direct-sun attenuation where the material shader uses it, records terrain height-debug draws through the same mesh-renderer/draw-list path, records a far-depth procedural sky fullscreen draw into remaining HDR scene-color pixels using shared depth, ends the scene render pass, records bloom from that completed HDR scene color into temporary pyramid targets, and then records tone mapping with the optional bloom input into the browser or native platform target. The sky shader combines analytic clear-sky radiance, sunrise/sunset warmth, a tighter warm sun disc/core with restrained sky-side halo, moon disc/phase, deterministic procedural stars, and a non-volumetric cloud layer controlled by `SkyWeather` coverage, storm, haze, wind, opacity, and precipitation hints. `Resources::create_*` only allocates labeled resources; the explicit texture, shader, material, and mesh `init_*` methods validate data and create GPU state. `Resources::load_model_resource` returns stable `Ptr<ModelResource>` observers and enqueues the target in the generic scheduler. The temporary `ModelResourceLoader` discovers glTF dependencies through generic blob requests, imports loaded documents into the same `ModelResource` object owned by `Resources`, and creates imported mesh/material/texture/shader resources through `Resources` with deterministic parent-URI `#` fragment labels. The demo scene creates a sun light entity, camera entity, terrain height-debug shader resources, one shared debug-plane mesh, one `R16Float` heightfield texture and material per generated terrain chunk, a white texture, opaque materials, a reusable cube mesh, a fallback player box, and stable terrain/player/large-box-field entities whose transforms are updated per frame. When the browser player model loads, the fallback player-box renderer is hidden and the imported skinned model supplies the visible player. The renderer owns the scene color target, depth target, shadow map target, shadow caster pass, opaque pass, sky pass, bloom pass, tone-map pass, transient render-object list, transient draw list, frame/draw/sky/tone-map/bloom/shadow uniform buffers, opaque fallback shadow texture/view/sampler, pipeline cache, resource counters, culling counters, shadow pass diagnostics, and WebGPU command recording for the current visual contract.
...
- `cpp/include/ofg/render/demo_scene.hpp` builds the generated demo resources and updates the per-frame large default validation scene objects.
- `cpp/include/ofg/scene/scene.hpp`, `light.hpp`, `environment.hpp`, and terrain headers expose the explicit scene view, scene light components, global environment state, and terrain data passed from `Game` toward `Renderer`.
- `cpp/include/ofg/render/bounds.hpp`, `frustum.hpp`, and `render_object.hpp` expose conservative render bounds, explicit culling plane sets, and bounded render-object extraction shared by camera culling and shadow-caster culling.
- `cpp/include/ofg/render/shadow_settings.hpp`, `shadow_cascade.hpp`, `shadow_frame_state.hpp`, `shadow_map_target.hpp`, `shadow_caster_pass.hpp`, and `shadow_diagnostics.hpp` expose cascaded shadow settings, split helpers, low-sun fade/clamp helpers, stable light-space matrices, texel snapping, cascade culling planes, opaque shadow uniform packing, depth texture-array ownership, depth-only caster rendering, and per-cascade pass diagnostics.
...
- `cpp/include/ofg/render/temp_buffer.hpp` exposes renderer-only reusable temporary render-target handles for post-effect scratch work.
- `cpp/include/ofg/render/bloom_settings.hpp` exposes CPU-side bloom settings, pyramid planning, threshold math, and uniform packing for the bloom post effect.
- `cpp/include/ofg/render/bloom_pass.hpp` exposes the BloomPass vertical slice: prefilter, downsample, upsample, diagnostics, and `BloomResult` handoff for `ToneMapPass`.
- `cpp/include/ofg/render/tone_map_pass.hpp` accepts an optional bloom input and owns a durable 1x1 fallback bloom texture so disabled bloom uses the same tone-map pipeline shape.
...
- The renderer requests no optional GPU features and creates durable resources during lifecycle creation, first render for a new material/shader/shadow-layout pipeline key, explicit mutation, shadow map resize, or renderer resize, not every ordinary steady-state frame.
- Compatible material bind-group layouts are shared by structural material binding layout so the opaque pipeline cache is bounded by shader/schema rather than by every material instance that binds a different texture.
- The opaque pass starts with a bounded 256-draw uniform capacity so the default terrain/debug scene and browser smoke camera exercises do not grow the draw-uniform buffer during ordinary steady-state frames.
- Surface/texture format may differ between browser and native targets, but the visual contract remains a tone-mapped procedural sky background, scene-owned terrain rendered through the draw-list path, an imported player model or fallback player box, and multiple colored cubes rendered through the opaque draw-list path.
...
- BrowserSmoke treats debug status JSON as the public runtime inspection interface and waits for `playerModelLoaded` before checking the default demo-scene counts, render-culling counts, shadow pass/cascade/caster diagnostics, steady-state renderer counters, active bloom diagnostics, and temp-buffer reuse diagnostics.
...
- The report JSON is the machine-readable contract for CI, human review, and coverage runs. It includes the C++ runtime debug-status snapshot so native artifacts preserve demo-scene, render-culling, shadow pass, bloom pass, and temp-buffer diagnostics alongside pixel coverage.
