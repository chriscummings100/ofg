...
This plan starts the procedural terrain system by making terrain real in the renderer. The first user-visible result is a rendered scene where C++ owns a scene-level `Terrain` object, generates deterministic addressable `TerrainChunk` data, writes a half-precision heightfield debug texture per chunk, and renders one debug quad per chunk at `y == 0`. The next visible result is a generated heightfield mesh rendered with a simple clay material while preserving the debug-plane path for later editor switching.

The goal is not to solve final terrain streaming yet. The goal is to establish the first stable terrain contract: a `Scene` owns terrain like it owns `Environment`, terrain chunks are addressable by `(LOD, X, Y, Z)`, C++ can generate the same heightfield every time, adjacent surface chunks line up, and the result is visible in the game before we try to reason about richer generation.
...
- [x] (2026-07-04 10:25+01:00) Removed browser-facing terrain debug/editor APIs from this plan; terrain editor work will come through a separate ImGui integration plan before editor implementation begins.
- [ ] Create the scene-owned C++ `Terrain` and `TerrainChunk` model with doctests.
- [ ] Generate sine-octave heightfields and render chunk debug quads with half-precision height textures.
...
- Decision: Milestone 2 adds generated heightfield meshes and renders them with a basic clay material, while keeping the debug-plane code.
  Rationale: The mesh is the useful terrain renderer path, but the debug texture plane remains valuable for later UI switching and generator inspection.
  Date/Author: 2026-07-04 / User

- Decision: Do not expose terrain debug/editor data through the browser facade in this plan.
  Rationale: Browser-facing terrain payloads would grow quickly and duplicate editor concerns. The terrain editor should be built through a separate ImGui integration plan before terrain editor work begins.
  Date/Author: 2026-07-04 / User

- Decision: Create and execute a separate ImGui integration plan before terrain editor work.
  Rationale: Terrain controls, visualization, and editor workflow should live in an in-engine editor surface rather than in TypeScript DOM or ad hoc WASM debug payloads.
  Date/Author: 2026-07-04 / User
...
- Decision: Untextured debug planes and clay-material heightfield meshes are acceptable for this plan.
  Rationale: The first important proof is chunk identity, generated height data, seams, scale, camera readability, and render integration. Materials, biome masks, triplanar shading, and erosion-driven textures can follow after terrain is visible.
  Date/Author: 2026-07-04 / Codex
...
This plan preserves `OFG-BOOT-004 Renderer Compatibility` and `OFG-BOOT-005 WebGPU Baseline`: browser and native rendering should remain on WebGPU-backed renderer paths, request no optional WebGPU features, and keep smoke-compatible sky, camera, player fallback/model, and opaque render behavior. Terrain may add a heightfield-debug shader path and later a clay material path, and it may change the visible scene contract, but screenshots and smoke thresholds must be updated deliberately if pixel classification changes. Half-precision heightfield textures must be implemented using WebGPU core features only; if the chosen half-float format imposes sampler restrictions, the implementation must use a compatible non-filtering path rather than requesting optional features.

This plan preserves `OFG-BOOT-006 Resource Lifetime`: terrain textures, debug-plane meshes, terrain mesh resources, shaders, and materials may be created through `Resources` during scene preparation or explicit terrain mutation, not recreated during every ordinary steady-state frame. Repeated rendering should reuse existing `Texture`, `Mesh`, `Shader`, and `Material` resources referenced by each `TerrainChunk`.
...
2. Add terrain headers and implementation:
       ...
       C:\dev\ofg\cpp\include\ofg\terrain\terrain_heightfield.hpp
       ...
       C:\dev\ofg\cpp\src\terrain\terrain_heightfield.cpp
...
3. Add native tests:
   ...
   The tests should cover `Terrain` ownership behavior, chunk map lookup, unique chunk ids, origin-centered `Terrain::tick(...)` generation, deterministic sine-octave height generation, neighbor edge agreement, invalid configuration errors, and later mesh counts.
...
5. Add or extend texture support for half-precision terrain heightfields:
   ...
   Prefer a narrow, well-tested `R16Float` or equivalent half-float path that works without optional WebGPU features. The debug shader maps the sampled signed height to red for negative and green for positive.
...
6. Add the terrain debug shader and flat chunk plane render path:
   ...
   Keep `Game::prepare_impl()` small by calling helpers rather than embedding chunk-generation details.
...
7. In Milestone 2, add heightfield meshing and clay rendering:
   ...
   Preserve the Milestone 1 debug-plane code path for later UI switching.
...
10. For visual validation during and before finalization:
   ...
   Report the printed local URL. Capture screenshots regularly for browser/render work. Store durable terrain screenshots under `C:\dev\ofg\artifacts\terrain-debug` and present them in chat.
...
Milestone 1 review scope is `Scene` terrain ownership, `Terrain` and `TerrainChunk` API shape, chunk id stability, sine-octave heightfield generation, half-float texture handling, debug shader integration, flat per-chunk debug rendering, CMake integration, C++ tests, and screenshots.

Milestone 2 review scope is heightfield mesh generation, clay material rendering, preservation of debug-plane code, resource lifetime, renderer compatibility, and screenshots proving visible terrain mesh.
...
- moving the generation origin across a chunk boundary causes `Terrain::tick(...)` to create the newly needed chunk ids without duplicating existing chunks;
...
- heightfield texture CPU data uses a half-precision representation or a tested conversion path into a half-precision WebGPU texture;
...
Terrain generation should be deterministic for the same `TerrainConfig`, `TerrainChunkId`, and generation origin, so tests and debug visualizations can be rerun without persistent setup. The initial terrain scene integration should create resources during terrain ticking and scene preparation; restarting the runtime or dev server should rebuild the same chunk set from source.
...
- one simple clay material for Milestone 2 heightfield meshes;
...
The C++ terrain interface should end with stable names close to:
    ...
    public:
        ...
        void generate_debug_texture();
        void generate_debug_plane_mesh();
        void generate_heightfield_mesh();
    };
...
Exact names may change if implementation finds a better local pattern, but the concepts should remain: scene-owned terrain, addressable 3D chunk id, fixed LOD0 chunk dimensions outside `TerrainConfig`, deterministic config owned by `Terrain`, origin-centered ticking through `Terrain`, sine-octave sampling through `Terrain`, half-float debug texture output referenced by `TerrainChunk`, and mesh output stored as chunk state in Milestone 2.
