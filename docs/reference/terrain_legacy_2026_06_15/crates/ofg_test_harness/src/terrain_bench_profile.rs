...
use terrain_core::{
    height_at, terrain_chunk_coord_containing_position, terrain_node_key,
    terrain_variant_for_preset, TerrainChunkCoord, TerrainLodBand, TerrainNodeKey,
    TerrainStreamConfig, TerrainStreamScheduler,
};
...
fn collect_stream_profile_scenarios(
    ...
    let mut scheduler = TerrainStreamScheduler::new(TerrainStreamConfig {
        ...
        terrain_seed: seed,
        terrain_variant: terrain_variant_for_preset(preset.code),
...
/// Returns stable fixed-offset LOD bands for profile comparison scenarios.
fn benchmark_lod_bands() -> Vec<TerrainLodBand> {
    vec![
        TerrainLodBand::fixed_offsets(
            0,
            STREAMING_HORIZONTAL_RADIUS,
            STREAMING_VERTICAL_CHUNK_OFFSETS.to_vec(),
        ),
        TerrainLodBand::fixed_offsets(1, 2, STREAMING_VERTICAL_CHUNK_OFFSETS.to_vec()),
        TerrainLodBand::fixed_offsets(2, 3, STREAMING_VERTICAL_CHUNK_OFFSETS.to_vec()),
        TerrainLodBand::fixed_offsets(3, 2, STREAMING_FAR_VERTICAL_CHUNK_OFFSETS.to_vec()),
        TerrainLodBand::fixed_offsets(4, 4, STREAMING_FAR_VERTICAL_CHUNK_OFFSETS.to_vec()),
    ]
}
