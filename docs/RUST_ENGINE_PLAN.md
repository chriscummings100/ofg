...
| 2026-06-02 | TypeScript render item packets retired | Deleted the generic compiled `RenderItemPacket` path. The temporary renderer adapter now consumes the Rust terrain packet source directly and submits terrain chunk IDs, mesh IDs, material IDs, and world matrices to `RustBrowserGame`; the app loop no longer assembles a TypeScript render item list. |
