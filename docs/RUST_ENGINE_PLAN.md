...
configures terrain material texture bindings through a terrain-specific facade
call, passes compact per-item world matrices, and applies a narrow browser
...
resource handles, the fixed terrain material recipe, material-to-texture
...
| 2026-06-03 | Terrain material definition moved to Rust browser facade | Deleted the compiled TypeScript `Material` model, removed material fields from the terrain packet store and worker streamer, and replaced the generic `upsertMaterial`/`destroyMaterial` browser API with a terrain-specific `upsertTerrainMaterial` call. TypeScript now supplies terrain texture IDs only; Rust owns the terrain material recipe and per-frame material selection for streamed terrain chunks. |
